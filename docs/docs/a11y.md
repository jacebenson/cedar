...
We built Cedar to make building websites more accessible (we write all the config so you don't have to), but Redwood's also built to help you make more accessible websites.
...
Even though Cedar looks for a `RouteAnnouncement` component first, you don't have to have one on every page—it's more than ok for the `<h1>` to be what's announced.
...
On page change, Cedar Router resets focus to the top of the DOM so that users can navigate through the new page.
