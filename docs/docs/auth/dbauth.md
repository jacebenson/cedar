...
> ```
>
> You'll need to let Cedar know what field you're using for your users' `id` and `username` fields In this case we're using `id` and `email`, so update those in the `authFields` config in `/api/src/functions/auth.js` (this is also the place to tell Cedar if you used a different name for the `hashedPassword` or `salt` fields):
>
> ```
> authFields: {
>   id: 'id',
...
The `LoginPage` generated by Cedar includes two new prompts on the login page, depending on the state of the user and whether they have registered their device yet or not:
