...
👷🏾 Heads up! Make sure that you're running node v20.17.0 or greater and yarn v4.1.1 or greater. If you need help getting set up or upgrading, you can refer to our documentation on using [nvm](https://cedarjs.com/docs/how-to/using-nvm) and [yarn](https://cedarjs.com/docs/how-to/using-yarn).
...
We've added support for Docker in Redwood. You can now deploy your Redwood app anywhere that supports docker with ease. We have a guide on how to do this [here](https://cedarjs.com/docs/docker).
...
You can no longer use the `server.config.js` file to configure your fastify server. Instead, you'll have to migrate to the new server file setup. This is documented [here](https://cedarjs.com/docs/canary/docker/#using-the-server-file).
